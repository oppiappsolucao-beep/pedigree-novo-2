...
def upload_foto_pet_to_drive(foto_pet, tutor_nome: str) -> str:
    ...
    Observação:
    A pasta precisa estar compartilhada como EDITOR com o e-mail da service account.
    """
    if not foto_pet:
        return ""
    ...
    # Testa se a service account consegue enxergar a pasta.
    ...
    if folder_check.status_code not in [200]:
        raise Exception(
            "A service account não conseguiu acessar a pasta do Drive. "
            f"Compartilhe a pasta com este e-mail como EDITOR: {service_email}. "
...
def image_to_base64(path: str) -> str:
    ...
    Se o arquivo não existir, retorna vazio para não quebrar o dashboard.
    """
    try:
